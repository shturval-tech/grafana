...
func adjustMetricFrame(frame *data.Frame, query *lokiQuery) {
...
func adjustLogsFrame(frame *data.Frame, query *lokiQuery) error {
	...
	idField, err := makeIdField(stringTimeField, frame.Fields[1], frame.Fields[2], frame.RefID)
	if err != nil {
		return err
	}
